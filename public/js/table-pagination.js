...
// -- Private data --
    ...
    let updateInfoText = (table, rowInfo, pageOptions) => {
        ...
        let text = `إظهار ${rowInfo.itemCountStart} إلى ${rowInfo.itemCountEnd} من ${rowInfo.totalItems} صف.`;
        text += `<br />`;
        text += `الصفحة ${rowInfo.pageNumber} من ${rowInfo.totalPages}`;
...
// Add navigation input
    ...
    let attachNavigationInput = (options) => {
        ...
        attachButton({
            ...
            text: onButtonTextRender.call(this, 'اﻹنتقال', settings.onRenderDescGoInput),
            title: onButtonTitleRender.call(this, 'اﻹنتقال إلى الصفحة', settings.onRenderDescGoInput),
        });
